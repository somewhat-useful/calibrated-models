...
conversation is held, whether a prediction head runs. That has a price, and it is worth
stating. The numbers come from this card, so the same model calibrated on a different
card is served under a different name, and a client holding qwen3.8-45k-q8 stops finding
it. The alternative was to name a profile for what it is for rather than what it is --
fast, long -- which survives a change of card and tells the person nothing about what
they get. This file takes the first.
...
from .place import CacheType, Settings
...
def names(key: str, chosen: Sequence[Settings]) -> tuple[Profile, ...]:
    ...
    """
    if len(chosen) == 1:
        return (Profile(key, chosen[0]),)

    return tuple(Profile(f"{key}-{_apart(settings)}", settings) for settings in chosen)


def _apart(settings: Settings) -> str:
    """What tells one profile of a model from the others: its window, cache and head.

    Nothing about placement appears. A dense model's layers are all on the card, and a
    mixture has one profile and never reaches here, so the count of offloaded experts
