...
card, less a share for the system. Written down by name, it is also room the weights may
not take: a mixture is placed within what is left after it. Half the memory, which is what an older version of this
...
System memory is the other bound the cards are one of. What a mixture leaves there may
not be more than this machine has for it -- what it has beyond the system's own share of
8 GiB, less a `cache_ram` written down by name -- because weights counted on but not held
are read off the disk for every token. A model needing more than that, however its
experts are moved, is not placed, and the run says so under its name.
